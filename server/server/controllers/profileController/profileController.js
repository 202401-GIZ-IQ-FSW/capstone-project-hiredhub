...
exports.createProfile = async (req, res) => {
    const { firstName, lastName, phoneNumber, location, resume, profilePicture } = req.body;
    const userId = req.user.id;
    try{
...
exports.addSavedJobs = async (req, res) => {
    ...
    try{
        ...
        res.status(200).json({ message: "Job saved successfully", savedJobs: profile.savedJobs });
    }catch(err){
        res.status(500).json({ error: err })
    }
}
